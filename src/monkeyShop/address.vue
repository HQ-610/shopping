<template>
	<div>
			...
			<div class="container">
				<div class="checkout-addr">
					...
					<div class="addr-list-wrap">
						<div class="addr-list">
							<ul>
								<li v-for="(item,index) in addressListLimit" v-bind:class="{'check':index == checkIndex}" @click="checkIndex=index;selectId=item.addressId">
									<dl>
										<dt>{{item.userName}}</dt>
									...
									<div class="addr-opration addr-set-default">
										<a class="addr-set-default-btn" @click="setDefault(item.addressId)" v-if="!item.isDefault"><i>Set default</i></a>
									</div>
						...
						<div class="shipping-addr-more">
							<a class="addr-more-btn up-down-btn" @click = "openList" v-bind:class="{'open':limit>3}">
								more
								<i class="i-up-down">
		...
		<model v-bind:mdShow="deleAddress" v-on:close="closeAddress">
			<p slot="message">
				您确定要删除这个地址吗？
			</p>
			<div slot="btnGroup">
				<a class="btn btn--m" @click="deleAddressConfirm">确认</a>
				<a href="javascipt:;" class="btn btn--m" @click="deleAddress=false">取消</a>
			</div>
		</model>
